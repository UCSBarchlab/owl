...
class AES(object):
    ...
    def encrypt_state_m(self, plaintext_in, key_in, reset):
        ...
        """
        if len(key_in) != 128 or len(plaintext_in) != 128:
            raise pyrtl.PyrtlError("AES key and plaintext should be length 128")

        ciphertext = pyrtl.Register(128, "ciphertext") #formerly plain_text
        round_key = pyrtl.Register(128, "round_key") #formerly key

        key_exp_in, add_round_in = (pyrtl.WireVector(len(plaintext_in)) for i in range(2))

        next_round = pyrtl.WireVector(4, 'next_round')
        round = pyrtl.Register(4, "round")
        round.next <<= next_round

        # round internal is used to match the spec's round count (in the round reg) to the implementation's existing
        # system here
        round_internal = pyrtl.WireVector(4, "round_internal")
        round_internal <<= pyrtl.select(round == pyrtl.Const(0, bitwidth=4), pyrtl.Const(0, bitwidth=4), round - 1)

        state = pyrtl.WireVector(4, "state")
        # state we're in is a function of the round and reset (provided state constants a priori)
        state <<= pyrtl.net_hole("state_hole", 2, round, reset)

        sub_out = self._sub_bytes(ciphertext)
        shift_out = self._shift_rows(sub_out)
        mix_out = self._mix_columns(shift_out)
        key_out = self._key_expansion(round_key, round_internal)
        # xor
        # add round out is cipher text
        add_round_out = self._add_round_key(add_round_in, key_exp_in)

        with pyrtl.conditional_assignment:
            #reset == 1
            #with state == state1:
            with state == pyrtl.net_hole("state1", 2):
                # next_counter |= 0
                next_round |= round + 1
                key_exp_in |= key_in  # to lower the number of cycles
                ciphertext.next |= add_round_out
                round_key.next |= key_in
                add_round_in |= plaintext_in

            with state == pyrtl.net_hole("state2", 2):
                #next_counter |= counter + 1
                next_round |= round + 1
                key_exp_in |= key_out
                ciphertext.next |= add_round_out
                round_key.next |= key_out
                add_round_in |= mix_out

            with state == pyrtl.net_hole("state3", 2):
                # round |= counter + 1
                next_round |= round + 1
                key_exp_in |= key_out
                ciphertext.next |= add_round_out
