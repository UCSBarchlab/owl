...
(define (Fetch pre ports)
  (Load (pre (ports "imem")) (extract 31 2 (pre (ports "pc")))))
...
(define (R-post pre post ports op)
  (let* ([ir (Fetch pre ports)]
         ...
         [Rrs1 (if
                (bvzero? rs1)
                (bv 0 32)
                (Load (pre (ports "rf")) rs1))]
         [Rrs2 (if
                (bvzero? rs2)
                (bv 0 32)
                (Load (pre (ports "rf")) rs2))])
    (begin
       (assert
         (equal? (post (ports "pc"))
                 (bvadd
       ...
       (assert (bveq (Load (post (ports "rf")) rd)
                     (apply op (list Rrs1 Rrs2))))
       (assert
         (equal? (vector-ref (post (ports "dmem")) 1)
                 (vector-ref (pre (ports "dmem")) 1))))))

;; clmul
(define (clmul-pre pre ports)
  (R-pre pre ports
         (bv #b0110011 7)
         (bv #x1 3)
         (bv #x05 7)))

(define (clmul-post pre post ports)
  (R-post pre post ports
          clmul))

;; clmulh
(define (clmulh-pre pre ports)
  (R-pre pre ports
         (bv #b0110011 7)
         (bv #x3 3)
         (bv #x05 7)))

(define (clmulh-post pre post ports)
  (R-post pre post ports
          clmulh))

;; andn
(define (andn-pre pre ports)
  (R-pre pre ports
         (bv #b0110011 7)
         (bv #b111 3)
         (bv #b0100000 7)))

(define (andn-post pre post ports)
  (R-post pre post ports
          andn))

;; orn
(define (orn-pre pre ports)
  (R-pre pre ports
         (bv #b0110011 7)
         (bv #b110 3)
         (bv #b0100000 7)))

(define (orn-post pre post ports)
  (R-post pre post ports
          orn))

;; xnor
(define (xnor-pre pre ports)
  (R-pre pre ports
         (bv #b0110011 7)
         (bv #b100 3)
         (bv #b0100000 7)))

(define (xnor-post pre post ports)
  (R-post pre post ports
          xnor))

;; rol
(define (rol-pre pre ports)
  (R-pre pre ports
         (bv #b0110011 7)
         (bv #b001 3)
         (bv #b0110000 7)))

(define (rol-post pre post ports)
  (R-post pre post ports
          rotatel))

;; ror
(define (ror-pre pre ports)
  (R-pre pre ports
         (bv #b0110011 7)
         (bv #b101 3)
         (bv #b0110000 7)))

(define (ror-post pre post ports)
  (R-post pre post ports
          rotater))

;; pack
(define (pack-pre pre ports)
  (R-pre pre ports
         (bv #b0110011 7)
         (bv #b100 3)
         (bv #b0000100 7)))

(define (pack-post pre post ports)
  (R-post pre post ports
          pack))

;; packh
(define (packh-pre pre ports)
  (R-pre pre ports
         (bv #b0110011 7)
         (bv #b111 3)
         (bv #b0000100 7)))

(define (packh-post pre post ports)
  (R-post pre post ports
          packh))
...
(define (zip-post pre post ports)
  (let* (
         [ir (Fetch pre ports)]
         [rd (extract 11 7 ir)]
         [rs1 (extract 19 15 ir)]
...
(define (unzip-post pre post ports)
  (let* (
         [ir (Fetch pre ports)]
         [rd (extract 11 7 ir)]
         [rs1 (extract 19 15 ir)]
...
(define (rev8-post pre post ports)
  (let* ([ir (Fetch pre ports)]
         [rd (extract 11 7 ir)]
         [rs1 (extract 19 15 ir)]
         [Rrs1 (if
...
(define (revb-post pre post ports)
  (let* ([ir (Fetch pre ports)]
         [rd (extract 11 7 ir)]
         [rs1 (extract 19 15 ir)]
         [Rrs1 (if
...
"andn" (list andn-pre andn-post)
"orn" (list orn-pre orn-post)
"xnor" (list xnor-pre xnor-post)
"rol" (list rol-pre rol-post)
"ror" (list ror-pre ror-post)
"rori" (list rori-pre rori-post)
"pack" (list pack-pre pack-post)
"packh" (list packh-pre packh-post)
"zip" (list zip-pre zip-post)
"unzip" (list unzip-pre unzip-post)
"rev8" (list rev8-pre rev8-post)
"revb" (list revb-pre revb-post)))
